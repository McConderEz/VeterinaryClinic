...
ORDER BY [Количество процедур] DESC;";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            myConnection.Open();
            var table = new DataTable();
            string sql = "SELECT [Районы].[Район города], [Ветеринарные клиники].[Номер регистрационного пункта], SUM([Процедуры].[Цена процедуры]) AS 'Выручка'" +
                       "FROM [Ветеринарные клиники]" +
                       "INNER JOIN [Процедуры] ON ([Ветеринарные клиники].[Номер регистрационного пункта]) = [Процедуры].[Номер регистрационного пункта]" +
                       "INNER JOIN [Районы] ON ([Ветеринарные клиники].[Код район города]) = [Районы].[Код района]" +
                       "WHERE YEAR([Процедуры].[Дата оказания помощи животному]) = 2023" +
                       "GROUP BY [Районы].[Район города], [Ветеринарные клиники].[Номер регистрационного пункта]"; ;
            SqlCommand command = new SqlCommand(sql, myConnection);
            SqlDataAdapter adapter = new SqlDataAdapter(command);
            table = new DataTable();
            adapter.Fill(table);
